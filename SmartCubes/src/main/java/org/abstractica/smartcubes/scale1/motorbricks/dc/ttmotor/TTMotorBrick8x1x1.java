...
public class TTMotorBrick8x1x1 implements Part
{
	public TTMotorBrick8x1x1()
	{
	...
	public Geometry3D getGeometry(JavaCSG csg, double scale, int angularResolution)
	{
		...
		Geometry3D topBlockCutout2 = csg.box3D(6*16, 4, 12, false);
		topBlockCutout = csg.union3D(topBlockCutout, topBlockCutout2);
		...
		//Create topblock
		Geometry3D topBlock = csg.box3D(6*16-8.2, 24, 12, false);
		Geometry3D topBlock2 = csg.box3D(6*16-0.2, 3.8, 12, false);
		topBlock = csg.union3D(topBlock, topBlock2);
		topBlock = csg.translate3DX(-24).transform(topBlock);
		topBlock = csg.difference3D(topBlock, motorCutout);
